...
function buildCharts(sample) {
  ...
  d3.json("samples.json").then((data) => {
    ...
    var yticks = otu_ids.map(function(id){
      return id;
    }).sort((a,b) => a.sample_values - b.sample_values).slice(0,10)
    //yticks need to be reversed for desired bar orientation
    yticks = yticks.reverse();
    //Create the yticks as OTU IDs.
    yticks1 = yticks.map(function(y){
      return `OTU ${y}`;
    });
    //xticks array and ylabels need to be reversed for desired bar orientation
    ...
    var selectedSample2 = metadata2.filter(s=>s.id == sample);
  
    // 2. Create a variable that holds the first sample in the metadata array.
    var firstSample2 = selectedSample2[0];

    // 3. Create a variable that holds the washing frequency.
    var washing_freq = parseFloat(firstSample2.wfreq);
    ...
    var gaugeLayout = { 
      width: 460, height: 430, margin: { t: 0, b: 0 } 
    };
